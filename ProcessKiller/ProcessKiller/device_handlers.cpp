...
NTSTATUS device_ioctl(PDEVICE_OBJECT device_object, PIRP irp) {
	...
	case KILL_PROCESS_IOCTL:
	{
		if (input_buffer_length != sizeof KillProcessArgs) {
			nt_status = STATUS_INVALID_PARAMETER;
			break;
		}
		auto args = static_cast<KillProcessArgs*>(irp->AssociatedIrp.SystemBuffer);
		nt_status = kill_process(*args);
	}
		break;
	default:
	{
